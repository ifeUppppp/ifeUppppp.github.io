...
function initAqiChartData() {
    ...
    for (var key in aqiSourceData) {
        //每一个循环代表一个城市对象
        var tempCity = aqiSourceData[key]; //当前city-data
        var keyArr = Object.getOwnPropertyNames(tempCity);//当前城市的所有数据的所有日期
        var tempMonth = keyArr[0].slice(5, 7);//（第一个日期的月份）当前月份month
        var weekInit = 4, weekCount = 0;//week计数
        
        
        //循环代表该城市的一条数据
        //每天 i++，星期n+1
        for (var i = 0; i < keyArr.length; i++, weekInit++) {
            count += tempCity[keyArr[i]];//该城市总数据
            mcount += tempCity[keyArr[i]];//month计数
            weekCount++;//天数
            //星期八  或者  数据最后一个   或者   月份有变动
        ...
        //这两个数为城市对应的week数据和month数据
        week[key] = singleWeek;//对象中的对象     week[city] = data;
        month[key] = singleMonth;             //month[key]
        //清空进行下一次遍历
        singleWeek = {};
        singleMonth = {};
    }
